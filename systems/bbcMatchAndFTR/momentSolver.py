...
class MomentSolver:
    ...
    def GetFoM_And_DFoM(self, index = -1, k0 = 1.0/1.3247499999999999):
        '''
        ...
        F_1: momentum/collimation of the beam dQ/dz = 0
        F_2: is the beam round in config. space?
        F_3: is the beam round in energy space?
        F_4: does the beam satisfy radial force balance?
        F_5: does the beam have minimal energy in the lab frame? (Derbenev scheme)
    ...
    def GetFoM_And_DFoM_Periodic(self,index=-1,k0=1.0/.6489):
        ...
        Y(9) - Ex [1]
        Y(10) - L [L]
        Ystr_vec = [r"\langle x^2 \rangle",r"\langle y^2 \rangle"
                    ,r"\langle x y \rangle",r"\langle x x' \rangle"
                    ,r"\langle y y' \rangle", r"\langle x y' \rangle"
                    ,r"\langle y x' \rangle", r"\langle x'^2 \rangle"
                    ,r"\langle y'^2 \rangle" , r"\langle x' y' \rangle"]
        ...
        # uarray = k0**2*np.array([k0**2, k0**2, k0**2,
        #                    1.0, 1.0, 1.0,
        #                    1.0/k0**2, 1.0/k0**2, 1.0/k0**2, 
        #                    k0**2]) # array of units for multiplication of stuff
        
        xxL = y[0,index] + y[1,index]; xx0 = y[0,0] + y[1,0]
        yyL = y[0,index] - y[1,index]; yy0 = y[0,0] - y[1,0]
        xyL = y[2,index]; xy0 = y[2,0]
        xxpL = (y[3,index] + y[4,index])/2.0 ; xxp0 = (y[3,0] + y[4,0])/2.0
        yypL = (y[3,index] - y[4,index])/2.0; yyp0 =(y[3,0] - y[4,0])/2.0
        xypL = (y[5,index] + y[9,index])/2.0; xyp0 = (y[5,0] + y[9,0])/2.0
        yxpL = (y[5,index] - y[9,index])/2.0; yxp0 = (y[5,0] - y[9,0])/2.0
        xpxpL = (y[6,index] + y[7,index])/2.0; xpxp0 = (y[6,0] + y[7,0])/2.0
        ypypL = (y[6,index] - y[7,index])/2.0; ypyp0 = (y[6,0] - y[7,0])/2.0
        xpypL = y[8,index]/2.0; xpyp0 =  y[8,0]/2.0
            
        # FoM = np.sum((y[:-1,index] - y[:-1,0])**2*uarray)/2.0
        # FoMp = np.array((y[:-1,index] - y[:-1,0])**2*uarray/2.0)  
        # FoMp = 0.5*np.array([
        #                     (xxL - xx0)**2*k0**4,
        #                     (yyL - yy0)**2*k0**4,
        #                     (xyL - xy0)**2*k0**4,
        ...
        # dFoM = dFoM_sign*np.array([
        #                         2.0*(y[0,index] - y[0,0])*k0**4,
        #                         2.0*(y[1,index] - y[1,0])*k0**4,
        #                         (y[2,index] - y[2,0])*k0**4,
        #                         0.5*(y[3,index])*k0**2,
        #                         0.5*(y[4,index])*k0**2,
        #                         0.5*(y[5,index])*k0**2,
        #                         0.5*(y[6,index] - y[6,0]),
        ...
        dFoM = dFoM_sign*np.array([
                                ...
                                2.0*(y[1,index] - y[1,0])*k0**4,
                                (y[2,index] - y[2,0])*k0**4,
                                0.5*(y[3,index] - y[3,0])*k0**2,
                                0.5*(y[4,index] - y[4,0])*k0**2,
                                0.5*(y[5,index] - y[5,0])*k0**2,
                                0.5*(y[6,index] - y[6,0]),
                                0.5*(y[7,index] - y[7,0]),
                                0.5*(y[8,index] - y[8,0]),
                                0.5*(y[9,index] - y[9,0])*k0**2
                                ])
...
class MomentSolverUtility:
    ...
    def GetInitialConditions(self, betax, betay, alphax, alphay, emitx, emity):
        ...
        # emittnace used is the geometric (Courant-Snyder) emittance, 
        # not RMS emittance which is what is provided in moment codes etc.
        emitx = emitx/4.0
        emity = emity/4.0
        Q_plus = 0.5*(betax*emitx + betay*emity)
        Q_minus = 0.5*(betax*emitx - betay*emity)
        Q_x = 0.0
        P_plus = -1.0*(alphax*emitx + alphay*emity)
        P_minus = -1.0*(alphax*emitx - alphay*emity)
        P_x = 0
        E_plus = (gammax*emitx + gammay*emity)
        E_minus = (gammax*emitx - gammay*emity)
        E_x = 0
        L = 0
        phi = 0
        return np.array([Q_plus,Q_minus,Q_x,P_plus,P_minus,P_x,E_plus,E_minus,E_x,L,phi])
...
class PlottingUtility:
    ...
    def PlotEnv(self, momObj=None, title=None):
        ...
        try:
            sfsol = xr[0] * 0.25 / np.max(np.abs(momObj.ksol))
    ...
    def plotParams(self, params):
        ...
        plt.figure() # plot the thing now
        for n in range(len(aa[0,:])):
            plt.plot(aa[:,n],linestyle=ls[n%4],label=r"$a_{%d}$"%(n+1))
    ...
    def plotfom(self, fomh, fomph=None):
        ...
        Ystr_vec = [r"\langle x^2 \rangle",r"\langle y^2 \rangle"
                    ...
                    ,r"\langle y x' \rangle", r"\langle x'^2 \rangle"
                    ,r"\langle y'^2 \rangle" , r"\langle x' y' \rangle"]
        Ystr_vec = [r"F_1",r"F_2",r"F_3",r"\varepsilon_1 F_4",r"\varepsilon_2 F_5"]
        
        

        ff = np.array([0, fomh[0]])
        ftmp = fomh[0]
        for i,val in enumerate(fomh):
            if (i>0 and val <= ftmp):
                ff = np.vstack([ff,[i,val]])
                ftmp = val
                
        plt.figure()
        plt.plot(ff[:,0],ff[:,1],marker=mark[0],label='FoM')
        plt.yscale("log")
        plt.xlabel(r"Iteration")
        plt.ylabel(r"$F_{\textrm{tot}}$ [1]")
        plt.grid(True)
        plt.tight_layout()

        plt.figure()
        if fomph is not None:
            ffp = np.zeros(( len(fomph),len(fomph[0]) ))
            for i,val in enumerate(fomph):
                ffp[i,:] = val
            for n in range(len(ffp[0,:])):
                #plt.plot(ff[:,0],ffp[ff[:,0].astype(int),n],marker=mark[n%4],linestyle=ls[n%4],label=r"$F_{%s}$"%(Ystr_vec[n]))
                plt.plot(ff[:,0],ffp[ff[:,0].astype(int),n],marker=mark[n%4],linestyle=ls[n%4],label=r"$%s$"%(Ystr_vec[n]))
        
        plt.xlabel(r"Iteration")
        plt.ylabel(r"$F_{i}$ [1]")
        plt.title(r"$F_{\textrm{tot}} = F_1 + F_2  + F_3 + \varepsilon_1 F_4 + \varepsilon_2 F_5$")
        plt.yscale("log")    
        plt.legend(loc="best")
        plt.grid(True)
        plt.tight_layout()
