...
def plotEnvelope(momObj):
    ...
    plt.plot(momObj.z,momObj.ksol * 1e-6) # plot scaled magnet plots in the same plot so we know where the magnets are in the lattice
    plt.plot(momObj.z,momObj.kquad * 1e-8) # plot scaled magnet plots in the same plot so we know where the magnets are in the lattice
    plt.grid(True)
...
###################################
# Opt parameters
from systems.bbcInitialMatch.optParameters import params,initialConditions,getParamArray,getParamObj
###################################
